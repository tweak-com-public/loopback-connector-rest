    ...
    settings.operations.forEach(function(op) {
      ...
      if (functions) {
        for (var f in functions) {
          ...
          // Inject invoke for this operation
          (function(builder) {
            dataSource['invoke'] = dataSource['invoke'] || {};
            DataAccessObject['invoke'] = DataAccessObject['invoke'] || {};

            // Inject the invoke function
            var invokeCFn = function() {
              return builder.invoke.apply(builder, arguments);
            };

            if (debug.enabled) {
              debug('Mixing in method: %s', f);
            }

            dataSource['invoke'][f] = invokeCFn;
            invokeCFn.accepts = [
              {name: 'request', type: 'object'},
            ];
            invokeCFn.shared = true;
            // dataSource.defineOperation(f, fn, fn);
            DataAccessObject['invoke'][f] = invokeCFn;
          })(builder);
        }
      }
    });
